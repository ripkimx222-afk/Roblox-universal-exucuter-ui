-- UniversalExecutorUI - final (movable edges, single-side resize, auto font scale)
...
New("TextLabel",{Parent=topBar, Size=UDim2.new(0.6,-24,1,0), Position=UDim2.new(0,16,0,0), BackgroundTransparency=1, Text="Electron", Font=Enum.Font.GothamBlack, TextSize=22, TextColor3=Color3.new(1,1,1), TextXAlignment=Enum.TextXAlignment.Left})
New("TextLabel",{Parent=topBar, Size=UDim2.new(0.4,-24,1,0), Position=UDim2.new(0.6,12,0,0), BackgroundTransparency=1, Text="by Hikmes0", Font=Enum.Font.Gotham, TextSize=14, TextColor3=Color3.fromRGB(220,220,255), TextXAlignment=Enum.TextXAlignment.Right})

local btnClose = New("TextButton", {Parent=topBar, Size=UDim2.new(0,36,0,36), Position=UDim2.new(1,-46,0,10), AnchorPoint=Vector2.new(1,0), BackgroundColor3=Color3.fromRGB(200,60,80), Text="✕", Font=Enum.Font.GothamBold, TextSize=18, TextColor3=Color3.new(1,1,1)})
New("UICorner",{CornerRadius=UDim.new(0,10)}, btnClose)
local btnMin = New("TextButton", {Parent=topBar, Size=UDim2.new(0,36,0,36), Position=UDim2.new(1,-92,0,10), AnchorPoint=Vector2.new(1,0), BackgroundColor3=Color3.fromRGB(100,100,100), Text="—", Font=Enum.Font.GothamBold, TextSize=18, TextColor3=Color3.new(1,1,1)})
...
local function MakeTabBtn(t,y)
	local b = New("TextButton",{Parent=leftTabs, Size=UDim2.new(1,0,0,44), Position=UDim2.new(0,0,0,y), BackgroundColor3=Color3.fromRGB(75,34,160), BorderSizePixel=0, Font=Enum.Font.Gotham, Text=t, TextSize=16, TextColor3=Color3.fromRGB(245,245,245)})
	New("UICorner",{CornerRadius=UDim.new(0,10)}, b)
	return b
end

local tabMainBtn = MakeTabBtn("Main", 0)
local tabExecBtn = MakeTabBtn("Executors", 56)
local tabSettingsBtn = MakeTabBtn("Settings", 112)
local tabDiscBtn = MakeTabBtn("Disclaimer", 168)
...
local function switchTab(btn, idx)
	for i=1,#pages do
		local p=pages[i]
		if i==idx then
			p.Visible=true
			p.Position=UDim2.new(1,0,0,0)
			TweenService:Create(p, TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.Out), {Position=UDim2.new(0,0,0,0)}):Play()
		else
			if p.Visible then
				TweenService:Create(p, TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.In), {Position=UDim2.new(-1,0,0,0)}):Play()
				task.delay(0.12, function() p.Visible=false end)
			end
		end
	end
	TweenService:Create(tabIndicator, TweenInfo.new(0.12,Enum.EasingStyle.Quad), {Position=UDim2.new(0,0,0,btn.Position.Y.Offset)}):Play()
	for _,tb in ipairs({tabMainBtn,tabExecBtn,tabSettingsBtn,tabDiscBtn}) do tb.BackgroundColor3 = (tb==btn) and Color3.fromRGB(150,90,220) or Color3.fromRGB(75,34,160) end
end
...
local codeBox = New("TextBox", {
	...
	Font=Enum.Font.Code, TextSize=14, TextColor3=Color3.fromRGB(245,245,255),
	PlaceholderText="Paste Lua here..."
})
New("UICorner",{CornerRadius=UDim.new(0,10)}, codeBox)

local executeButton = New("TextButton", {Parent=mainPage, Size=UDim2.new(0,140,0,44), Position=UDim2.new(0,18,1,-64), BackgroundColor3=Color3.fromRGB(110,50,220), Font=Enum.Font.GothamBold, Text="Execute", TextSize=16, TextColor3=Color3.new(1,1,1)})
New("UICorner",{CornerRadius=UDim.new(0,10)}, executeButton)

local statusLabel = New("TextLabel", {Parent=mainPage, Size=UDim2.new(1,-200,0,26), Position=UDim2.new(0,170,1,-54), BackgroundTransparency=1, Text="Status: Ready", Font=Enum.Font.Gotham, TextSize=14, TextColor3=Color3.fromRGB(220,220,240), TextXAlignment=Enum.TextXAlignment.Left})
...
for i,name in ipairs(bases) do
	local btn = New("TextButton", {Parent=execPage, Size=UDim2.new(1,-28,0,44), Position=UDim2.new(0,14,0,(i-1)*54+14), BackgroundColor3=Color3.fromRGB(95,45,170), Font=Enum.Font.Gotham, TextSize=16, TextColor3=Color3.new(1,1,1)})
	New("UICorner",{CornerRadius=UDim.new(0,8)}, btn)
	if not isAvailable(name) then btn.Text = name.." (not available)"; btn.BackgroundColor3 = Color3.fromRGB(70,40,80); btn.AutoButtonColor=false else btn.Text = name end
	...
	btn.MouseButton1Click:Connect(function()
		if not isAvailable(name) then
			...
			for _,b in ipairs(baseButtons) do if b.Text:match("^XENO") then b.BackgroundColor3=Color3.fromRGB(150,90,220); break end end
			switchTab(tabMainBtn,1); return
		end
		selectedBase = name
		for _,b in ipairs(baseButtons) do b.BackgroundColor3 = Color3.fromRGB(95,45,170) end
...
local function MakeSlider(parent,y,label,minv,maxv,init)
	...
	local lbl = New("TextLabel",{Parent=holder, Size=UDim2.new(0.34,0,1,0), Position=UDim2.new(0,6,0,0), BackgroundTransparency=1, Font=Enum.Font.Gotham, TextSize=14, Text=label, TextColor3=Color3.fromRGB(235,235,240), TextXAlignment=Enum.TextXAlignment.Left})
	...
	local dragging=false; local ref=nil
	...
	rail.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=true; ref=input; setFromX(input.Position.X) end end)
	rail.InputChanged:Connect(function(input) if dragging and input==ref then setFromX(input.Position.X) end end)
	rail.InputEnded:Connect(function(input) if input==ref then dragging=false; ref=nil end end)
	UserInputService.InputChanged:Connect(function(input) if dragging and input==ref and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then setFromX(input.Position.X) end end)
	valBox.FocusLost:Connect(function() local num = tonumber(valBox.Text) or init; num = math.clamp(num,minv,maxv); local frac=(num-minv)/(maxv-minv); knob.Size=UDim2.new(frac,0,1,0); valBox.Text = ("%d"):format(num) end)
	return { get = function() return tonumber(valBox.Text) or init end, set = function(v) valBox.Text = tostring(math.floor(math.clamp(v,minv,maxv)+0.5)); local frac=(math.clamp(v,minv,maxv)-minv)/(maxv-minv); knob.Size=UDim2.new(frac,0,1,0) end, isDragging = function() return dragging end }
end
...
updatePreview()
applyBtn.MouseButton1Click:Connect(function() local r,g,b=aSlider and rSlider.get() or 34, gSlider.get(), bSlider.get(); local a=aSlider.get(); main.BackgroundColor3=Color3.fromRGB(r,g,b); main.BackgroundTransparency = 1 - a/100; updatePreview() end)

local focusOffsetEnabled=false
focusToggle.MouseButton1Click:Connect(function() focusOffsetEnabled = not focusOffsetEnabled; focusToggle.Text = "Focus offset: "..(focusOffsetEnabled and "ON" or "OFF"); focusToggle.BackgroundColor3 = focusOffsetEnabled and Color3.fromRGB(95,35,200) or Color3.fromRGB(70,70,70) end)
resetPosBtn.MouseButton1Click:Connect(function() local vp = workspace.CurrentCamera.ViewportSize; main.Position = UDim2.new(0, math.floor((vp.X - main.AbsoluteSize.X)/2), 0, math.floor((vp.Y - main.AbsoluteSize.Y)/2)) end)

local isRussian = true
function setLanguage(russian)
	isRussian = russian
	if isRussian then
		tabMainBtn.Text = "Главная"; tabExecBtn.Text="Инжекторы"; tabSettingsBtn.Text="Настройки"; tabDiscBtn.Text="Дисклеймер"
		executeButton.Text = "Выполнить"; langToggle.Text="Язык: RU"; resetPosBtn.Text="Сбросить позицию"; applyBtn.Text="Применить"
		focusToggle.Text = "Смещение фокуса: "..(focusOffsetEnabled and "ВКЛ" or "ВЫКЛ"); statusLabel.Text="Статус: Готов"
	else
		tabMainBtn.Text="Main"; tabExecBtn.Text="Executors"; tabSettingsBtn.Text="Settings"; tabDiscBtn.Text="Disclaimer"
		executeButton.Text="Execute"; langToggle.Text="Language: EN"; resetPosBtn.Text="Reset position"; applyBtn.Text="Apply"
		focusToggle.Text="Focus offset: "..(focusOffsetEnabled and "ON" or "OFF"); statusLabel.Text="Status: Ready"
	end
end
langToggle.MouseButton1Click:Connect(function() setLanguage(not isRussian) end)

local discPage = pages[4]
local disclaimerLabel = New("TextLabel", {Parent=discPage, Size=UDim2.new(1,-24,1,-24), Position=UDim2.new(0,12,0,12), BackgroundTransparency=1, Text="Скрипт создан исключительно в развлекательных целях\nЗлоупотреблять им нельзя\nВерсия: Beta — со временем он улучшится", TextWrapped=true, Font=Enum.Font.Gotham, TextSize=16, TextColor3=Color3.new(1,1,1), TextXAlignment=Enum.TextXAlignment.Left, TextYAlignment=Enum.TextYAlignment.Top})
...
do
	local resizing=false; local startPos, startSize, refInput
	resizer.InputBegan:Connect(function(input)
		if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
			resizing=true; refInput=input; startPos=input.Position; startSize={X=main.Size.X.Offset, Y=main.Size.Y.Offset}
			input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then resizing=false; saveState() end end)
		end
	end)
...
local executors = {
	["XENO"] = function(code) if syn and syn.protect_gui then pcall(function() syn.protect_gui(screen) end) end; if type(loadstring)=="function" then pcall(function() loadstring(code or "")() end) end; return "XENO" end,
	["DELTA X"] = function(code) if delta and type(delta.execute)=="function" then pcall(function() delta.execute(code or "") end); return "DELTA X" end; if type(loadstring)=="function" then pcall(function() loadstring(code or "")() end) end; return "Fallback" end,
	["KRNL"] = function(code) if KRNL and type(KRNL.execute)=="function" then pcall(function() KRNL.execute(code or "") end); return "KRNL" end; if type(loadstring)=="function" then pcall(function() loadstring(code or "")() end) end; return "Fallback" end,
	["Arceus X"] = function(code) if ArceusX and type(ArceusX.execute)=="function" then pcall(function() ArceusX.execute(code or "") end); return "Arceus X" end; if type(loadstring)=="function" then pcall(function() loadstring(code or "")() end) end; return "Fallback" end,
	["Fluxus"] = function(code) if fluxus and type(fluxus.execute)=="function" then pcall(function() fluxus.execute(code or "") end); return "Fluxus" end; if type(loadstring)=="function" then pcall(function() loadstring(code or "")() end) end; return "Fallback" end
}
...
local function makeDraggable(zone)
	local dragging=false; local start, startPos, curInput
	zone.InputBegan:Connect(function(input)
		if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
			dragging=true; start = input.Position; startPos = main.Position
			input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false; saveState() end end)
		end
	end)
	zone.InputChanged:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch then curInput = input end end)
	UserInputService.InputChanged:Connect(function(input)
...
do
	...
	toggle.InputBegan:Connect(function(input)
		if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
			dragging=true; start = input.Position; startPos = toggle.Position
			input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false; saveState() end end)
		end
	end)
	toggle.InputChanged:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch then curInput=input end end)
	UserInputService.InputChanged:Connect(function(input)
		if dragging and input==curInput then
			local delta = input.Position - start
			local vp = workspace.CurrentCamera.ViewportSize
...
local function openUI()
	...
	main.Visible = true
	TweenService:Create(main, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, savedW, 0, savedH)}):Play()
end

local function closeUI()
	local savedW = main.Size.X.Offset; local savedH = main.Size.Y.Offset
	TweenService:Create(main, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size=UDim2.new(0,28,0,28)}):Play()
	task.delay(0.16, function() main.Visible=false; main.Size=UDim2.new(0,savedW,0,savedH) end)
	saveState()
end

toggle.MouseButton1Click:Connect(function() if main.Visible then closeUI() else openUI() end end)
...
local function updateFontSizes()
	...
	for _,tb in ipairs({tabMainBtn,tabExecBtn,tabSettingsBtn,tabDiscBtn}) do if tb then tb.TextSize = math.clamp(math.floor(h * 0.028), 12, 18) end end
end
