...
local gui = New("ScreenGui", {Name="EnhancedSafeUI_v2", ResetOnSpawn=false, IgnoreGuiInset=true}, playerGui)
...
local grad = New("UIGradient", {Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(98,40,210)), ColorSequenceKeypoint.new(1, Color3.fromRGB(35,10,120))}, Rotation=28}, main)
...
local execBox = New("TextBox", {Parent=pages[1], Size=UDim2.new(1,-24,0,260), Position=UDim2.new(0,12,0,12), BackgroundColor3=Color3.fromRGB(28,12,65), BackgroundTransparency=0.04, ClearTextOnFocus=false, MultiLine=true, TextWrapped=true, Font=Enum.Font.Gotham, TextSize=14, TextColor3=Color3.fromRGB(250,250,250), PlaceholderText="Paste Lua for local testing (simulation only)"}, pages[1])
...
local function clampToViewport(frame)
	...
	local nx = math.clamp(x, 0, math.max(0, vp.X - w))
	local ny = math.clamp(y, 0, math.max(0, vp.Y - h))
...
local function openFromCenter()
	local savedW = player:GetAttribute("ESUI_MainW") or main.Size.X.Offset
	local savedH = player:GetAttribute("ESUI_MainH") or main.Size.Y.Offset
	local savedX = player:GetAttribute("ESUI_MainXOff") or main.Position.X.Offset
	local savedY = player:GetAttribute("ESUI_MainYOff") or main.Position.Y.Offset
	local targetSize = UDim2.new(0, savedW, 0, savedH)
	local targetPos = UDim2.new(0, savedX, 0, savedY)

	local startSize = UDim2.new(0,24,0,24)
	local centerPos = centerPositionFor(targetSize)
	main.Size = startSize
	main.Position = centerPos + UDim2.new(0, math.floor((targetSize.X.Offset - startSize.X.Offset)/2)*-1, 0, math.floor((targetSize.Y.Offset - startSize.Y.Offset)/2)*-1)
	main.Visible = true
	TweenService:Create(main, TweenInfo.new(0.26, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = targetSize, Position = targetPos}):Play()
end

local function closeToCenter()
	local curSize = main.Size
	local curPos = main.Position
	local endSize = UDim2.new(0,24,0,24)
	local centerPos = centerPositionFor(curSize)
...
loadState()
RunService.Heartbeat:Connect(function() preview.BackgroundTransparency = main.BackgroundTransparency end)
...
execBtn.MouseButton1Click:Connect(function()
	...
	print("[SIMULATION] preview:", string.sub(code,1,400))
...
execBox.FocusLost:Connect(function()
	saveState()
	local mx = player:GetAttribute("ESUI_MainXOff") or main.Position.X.Offset
	local my = player:GetAttribute("ESUI_MainYOff") or main.Position.Y.Offset
	main.Position = UDim2.new(0, mx, 0, my)
end)

toggle:GetPropertyChangedSignal("Position"):Connect(function() saveState() end)
main:GetPropertyChangedSignal("Position"):Connect(function() saveState() end)
main:GetPropertyChangedSignal("Size"):Connect(function() saveState() end)
